...
import { dbOcupacionToSlot, OccupiedSlot } from "../services/adapters";
...
export function BookingWizard() {
  ...
  const { resources, blocks, addReservation } = useAppContext();
  ...
  // Ocupación real de TODOS los usuarios para el día elegido (vía RPC obtener_ocupacion,
  // que esquiva el RLS que oculta los turnos ajenos). reservations del context no alcanza:
  // a un miembro normal solo le trae las suyas.
  useEffect(() => {
    if (!selectedDate) return;
    const dateStr = format(selectedDate, "yyyy-MM-dd");
    TurnoService.obtenerOcupacionPorFecha(dateStr)
      .then(raw => setOccupiedSlots(raw.map(dbOcupacionToSlot)))
  ...
  const isSlotAvailable = (resourceId: string, time: string, date: Date) => {
    ...
    const isReserved = occupiedSlots.some(o => {
      if (o.resourceId !== resourceId) return false;
      const resStart = parseInt(o.startTime.split(':')[0], 10);
      const resEnd = parseInt(o.endTime.split(':')[0], 10);
      const slotHour = parseInt(time.split(':')[0], 10);
