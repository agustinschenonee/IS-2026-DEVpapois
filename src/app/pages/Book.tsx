...
export function BookingWizard() {
  ...
  const { resources, reservations, blocks, addReservation, currentUser } = useAppContext();
  ...
  const isSlotAvailable = (resourceId: string, time: string, date: Date) => {
    ...
    // Prevenir reservas de horarios que ya pasaron en el día de hoy
    const now = new Date();
    const todayStr = format(now, "yyyy-MM-dd");
    if (dateStr === todayStr) {
      const currentHour = now.getHours();
      const slotHour = parseInt(time.split(':')[0], 10);
      // Si la hora de la reserva es menor o igual a la hora actual, no está disponible
...
const handleConfirm = async () => {
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 flex flex-col lg:flex-row gap-8 transition-colors duration-300">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 p-6 md:p-8 min-h-[450px] transition-colors relative">
          ...
          {step === 1 && (
            <div className="flex flex-col items-center animate-in fade-in zoom-in duration-300">
              ...
              <div className="bg-gray-50 dark:bg-gray-800/50 p-4 md:p-6 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-inner flex justify-center hover:border-emerald-200 dark:hover:border-emerald-900/50 transition-colors w-full max-w-md">
                <style>{`
                  .rdp {
                    --rdp-cell-size: 44px;
              ...
              {selectedType === "room" && (
                <div className="w-full max-w-md mx-auto animate-in fade-in zoom-in duration-300 bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800 p-1.5 rounded-full border border-gray-200 dark:border-gray-700">
                    <button 
                      onClick={() => setCapacityFilter(Math.max(1, capacityFilter - 1))}
                      className="w-10 h-10 rounded-full bg-white dark:bg-gray-700 shadow-sm flex items-center justify-center text-gray-900 dark:text-white font-bold hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
                    >-</button>
                    <span className="text-xl font-black text-gray-900 dark:text-white w-8 text-center">{capacityFilter}</span>
                    <button 
                      onClick={() => setCapacityFilter(capacityFilter + 1)}
                      className="w-10 h-10 rounded-full bg-white dark:bg-gray-700 shadow-sm flex items-center justify-center text-gray-900 dark:text-white font-bold hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
                    >+</button>
                  </div>
                </div>
          ...
          {step === 3 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="flex flex-col items-center mb-8">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-center">Mostrando espacios para {selectedDate && <strong className="text-gray-900 dark:text-white capitalize">{format(selectedDate, "EEEE, d 'de' MMMM", { locale: es })}</strong>}.</p>
              </div>
                                ...
                                {TIME_SLOTS.map(time => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      title={!isAvailable ? "Este horario no está disponible" : "Seleccionar horario"}
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        toggleSlot(resource.id, time);
                                      }}
              ...
              <div className="w-full bg-white dark:bg-gray-900 border-2 border-emerald-100 dark:border-emerald-900/50 rounded-[2rem] p-6 md:p-8 shadow-lg hover:shadow-xl transition-shadow">
                <div className="flex items-center gap-5 border-b border-gray-100 dark:border-gray-800 pb-6 mb-6">
                  <div className="h-20 w-20 bg-gray-100 dark:bg-gray-800 rounded-2xl overflow-hidden shrink-0 shadow-inner">
                     <ImageWithFallback 
                        src={selectedResourceDetails?.imageUrl || ''} 
                        alt="Espacio" 
                ...
                <dl className="space-y-4 md:space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50 gap-2">
                    ...
                    <dd className="text-gray-900 dark:text-white font-bold sm:text-right text-lg ml-14 sm:ml-0 flex sm:flex-col items-center sm:items-end gap-2 sm:gap-0">
                      ...
                      <div className="text-xs text-emerald-600 dark:text-emerald-400 font-bold bg-emerald-50 dark:bg-emerald-900/30 inline-block px-2 py-0.5 rounded-md mt-0 sm:mt-0.5">
                        {selectedSlots.length} horas
                      </div>
                    </dd>
                  </div>
                ...
                <div className="mt-8 flex items-start gap-3 text-gray-500 dark:text-gray-400 text-sm bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl border border-blue-100 dark:border-blue-900/20">
        ...
        <div className="mt-8 flex justify-between items-center px-2">
          ...
          {step < 4 ? (
            ...
            <button
              ...
              className="flex items-center px-6 md:px-10 py-3 md:py-3.5 border border-transparent shadow-lg text-sm md:text-base font-bold rounded-2xl text-white bg-emerald-600 hover:bg-emerald-700 hover:scale-105 transition-all duration-300 animate-pulse hover:animate-none"
            >
              ¡Confirmar!
      ...
      {/* Right Sidebar - Real-time Summary (Hidden on small screens) */}
      <div className="w-full lg:w-80 shrink-0 hidden lg:block">
        <div className="sticky top-24 bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 p-6 overflow-hidden">
          ...
          <h3 className="font-extrabold text-lg text-gray-900 dark:text-white mb-6 flex items-center gap-2">
            Tu Selección
          </h3>
